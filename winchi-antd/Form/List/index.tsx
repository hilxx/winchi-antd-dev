...
const FormList: Model = ({
  ...
  const queryColumnEnum = processEnum((c, index) => {
    setColumns([...columns.slice(0, index), c, ...columns.slice(index + 1)]);
  });
  ...
  return (
    <div className={`${styles.list} ${className}`} style={{ width }}>
      ...
      <main>
        <Form.List name={`${dataIndex}`} {...formListProps} initialValue={wcInitVal}>
          {(fields, { add, remove }) => (
            <>
              {fields.map((filed, index) => (
                <Space key={filed.key} className={styles.space} align="center">
                  {columns?.map((c) => (
                    <ResolveChidren
                      hide={
                        typeof c.hideForm === 'function'
                          ? c.hideForm(formValues, index)
                          : c.hideForm
                      }
                      key={`${c.dataIndex}`}
