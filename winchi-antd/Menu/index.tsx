...
const WcMenu: Model = ({
 ...
 useEffect(() => {
  ...
  promise()?.then(
   Wc.sep(
    setOptions,
    v => selectHandle(v)({ keyPath: _findFirstKeyPath(v) }),
   )
  )
 }, [options_])

 const selectHandle: AF = os => R.compose(
  onSelect,
  R.reverse as AF,
...
export const resolveWcMenuOption = ({
 children,
 ...props
}: WcMenuOption) =>
 children ? (
  <SubMenu {...props}>
   {
    children.map(c => resolveWcMenuOption(c))
   }
  </SubMenu>
...
const _keyPathBackOption = R.curry(
 (options: WcMenuOption[], keys: string[]) => {
